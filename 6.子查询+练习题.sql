...
SELECT employee_id,last_name,salary
FROM employees e1,
(SELECT department_id,AVG(salary) avg_sal FROM employees GROUP BY department_id) dept_avg_sal
...
SELECT d.*,(SELECT AVG(salary) FROM employees WHERE department_id = d.`department_id`) dept_avg_sal
FROM departments d
WHERE department_id = (
			SELECT department_id
			FROM employees
